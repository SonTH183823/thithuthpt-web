...
function ExamInfoHistory({exam, item}) {
  ...
  const aaaa = () => {
    if (exam.subject === 9) {
      return `Số câu đúng ${item.numberListeningQuestionRight} Listening - ${item.numberReadingQuestionRight} Reading`
    } else {
      return `Số câu đúng ${item.numberQuestionRight}/${exam.numberQuestion} câu`
    }
  }
  return (
    <div className={"p-2"}>
      ...
      <div className={'bg-base-200 rounded-xl p-3 flex'}>
        ...
        <div>
          <div className="flex items-center text-primary flex-col border border-primary rounded-md">
            <div
              className={'text-white bg-primary p-2 uppercase text-sm'}>{exam.subject === 9 ? 'Kết quả' : 'Điểm số'} < /div>
            <div className="font-bold text-xl p-2">{(item.point).toFixed(1)}</div>
          </div>
        </div>
      </div>
